...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f122mX;

    /* renamed from: mY */
    private float[] f123mY;
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f122mX = new float[min];
            this.f123mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
                this.f122mX[i2] = fArr[0];
                this.f123mY[i2] = fArr[1];
            }
            if (Math.abs(this.f122mX[0]) <= 1.0E-5d && Math.abs(this.f123mY[0]) <= 1.0E-5d) {
                int i3 = min - 1;
                if (Math.abs(this.f122mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f123mY[i3] - 1.0f) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f122mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb.append(this.f122mX[0]);
            sb.append(",");
            sb.append(this.f123mY[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f122mX[i6]);
            sb.append(",");
            sb.append(this.f123mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f122mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f122mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f122mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f123mY[i];
        }
        float[] fArr2 = this.f123mY;
        float f3 = fArr2[i];
